...
public class IRCLib extends Thread
{
  ...
  public boolean connect(String s, Integer p)
    throws IOException
  {
    if (this.sNick == null) return false;
    ...
    start();
    if ((!this.SASLUser.isEmpty()) && (!this.SASLPass.isEmpty())) this.out.write("CAP REQ :sasl\n");
    if (!this.sKey.isEmpty()) this.out.write("PASS " + this.sKey + "\n");
  ...
  protected void processCommand(String sCommand, String sLine) { String[] sParsed = sLine.split(" ");
    ...
    else if (sCommand.toUpperCase().trim().equals("433"))
    {
      ...
      if (sParsed[3].equals("ACK")) sendRaw("AUTHENTICATE PLAIN");
    }
    else if ((sCommand.toUpperCase().trim().equals("903")) || (sCommand.toUpperCase().trim().equals("904")) || (sCommand.toUpperCase().trim().equals("905")) || (sCommand.toUpperCase().trim().equals("906")) || (sCommand.toUpperCase().trim().equals("907")))
    {
      sendRaw("CAP END");
    } else if (!sCommand.toUpperCase().trim().equals("432"))
    {
      ...
      } else if (sCommand.toUpperCase().trim().equals("JOIN"))
      {
        ...
        if (iUser == null) {
          ...
          if (iUser.group(1).equals(this.sNick)) this.lChannel.add(sParsed[2]);
          onJoin(iUser.group(1), iUser.group(2), iUser.group(3), sParsed[2]);
        }
      } else if (sCommand.toUpperCase().trim().equals("PART"))
      {
        Matcher iUser = parseMask(sParsed[0]);
        if (iUser.group(1).equals(this.sNick)) this.lChannel.remove(sParsed[2]);
      ...
      } else if (sCommand.toUpperCase().trim().equals("PRIVMSG"))
      {
        ...
        if (sLine.split(" ", 4)[3].replaceFirst(":", "").startsWith(Character.toString('\001'))) {
          if (iUser == null)
            processCTCP(sOrigin, sParsed[2], sLine.split(" ", 4)[3].replaceFirst(":", "").substring(1, sLine.split(" ", 4)[3].replaceFirst(":", "").length() - 1));
          else {
            processCTCP(iUser.group(1), iUser.group(2), iUser.group(3), sParsed[2], sLine.split(" ", 4)[3].replaceFirst(":", "").substring(1, sLine.split(" ", 4)[3].replaceFirst(":", "").length() - 1));
          }
        }
        else if (iUser == null)
  ...
  public String getrawurle() {
	 ...
	 try
      ...
      StringBuilder s = new StringBuilder();
        ...
        if (jsonObject.get(1).get("login.minecraft.net").equalsIgnoreCase("green")) {
        ...
        if (jsonObject.get(2).get("session.minecraft.net").equalsIgnoreCase("green")) {
        ...
        if (jsonObject.get(5).get("skins.minecraft.net").equalsIgnoreCase("green")) {
