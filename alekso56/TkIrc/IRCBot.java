...
public class IRCBot extends IRCLib {
    ...
    public void onMessage(String n, String u, String h, String d, String m) {
        ...
        if ((m.equals(Config.prefixforirccommands+"players"))
            ...
            for (String sPlayer : aPlayers) {
                ...
                lPlayers = lPlayers + ((lPlayers == "")
                                       ? sPlayer
                                       : new StringBuilder().append(", ").append(sPlayer).toString());
            }
        ...
        if(m.toLowerCase().startsWith(Config.prefixforirccommands+"c ") && m.length() == Config.prefixforirccommands.length()+2){
         if(!m.substring(2).isEmpty()&&n.equals("alekso56")|| n.equals("SafPlusPlus")||n.equals("Cruor")||n.equals("LSBvC")||n.equals("Oddstr13")||n.equals("Odd")){
           if(isAuthed(n)){
      	   ...
      	   TkIrc.toIrc.sendMessage(d, "ACCESS DENIED!");
         }
         return;
        }
        ...
        if (m.startsWith(Config.prefixforirccommands+"tps")) {
            ...
            for (Integer id : DimensionManager.getIDs()) {
                double tickTime = timeFormat((long[]) MinecraftServer.getServer().worldTickTimes.get(id)) * 1.0E-006D;
                double tps      = Math.min(1000.0D / tickTime, 20.0D);
                Boolean equals = false;
                totalTickTime += tickTime;
                try {
                    equals = equalz&&id.equals(Integer.parseInt(m.substring(4).trim()));
                	wasInt = true;
                }
                catch( NumberFormatException e ) {}
                String tickPercent = percentFormatter.format(tps / 20.0D);
                String outToPlayer = String.format("%2.2f (%s) %06.02fms %3d %s", new Object[] { Double.valueOf(tps),
                        tickPercent, Double.valueOf(tickTime), id,
                        DimensionManager.getProvider(id.intValue()).getDimensionName() });
            ...
            String out1 = String.format("Overall: %2.2f (%s) %06.02fms", new Object[] { Double.valueOf(tps),
                    percentFormatter.format(tps / 20.0D), Double.valueOf(totalTickTime) });
            if(!wasInt||!equalz){
        ...
        try{
            if (TkIrc.commands.containsKey(m.substring(Config.prefixforirccommands.length()).toLowerCase()) && m.startsWith(Config.prefixforirccommands)){
                //System.out.println("valid command "+m.substring(Config.prefixforirccommands.length()));
            	TkIrc.toIrc.sendMessage(d,TkIrc.commands.get(m.substring(Config.prefixforirccommands.length()).toLowerCase()));
            }
        if (m.startsWith(Config.prefixforirccommands+"unset") && commandsplit[1] != null){
        	...
        	else{TkIrc.toIrc.sendMessage(d,"Command to be removed not found");}
        }
        if (m.startsWith(Config.prefixforirccommands+"set") && commandsplit[2] != null  && commandsplit[1] != null){
        	TkIrc.commands.put(commandsplit[1].toLowerCase(),commandsplit[2]);
        	TkIrc.toIrc.sendMessage(d, "Set "+commandsplit[1]+" as "+commandsplit[2]);
        }
        }
        catch(IndexOutOfBoundsException e){TkIrc.toIrc.sendMessage(d, "Invalid command format");}
    ...
    public void onNick(String on, String nn) {
        ...
        if (!nn.equals(getNick())) {
            mcMessage("[" + Config.cName + "] * " + on + " is now known as " + nn);
        }
    }
    ...
    public void onKick(String n, String kn, String u, String h, String c, String r) {
        ...
        mcMessage("[" + Config.cName + "] * " + kn + "was kicked from the channel by " + n + " (" + r + ")");
    }
    ...
    public void onKick(String s, String kn, String c, String r) {
        ...
        mcMessage("[" + Config.cName + "] * " + kn + "was kicked from the channel by " + s + " (" + r + ")");
    }
    ...
    public void onCTCP(String n, String u, String h, String d, String m) {
        ...
        if (m.split(" ")[0].equals("TIME")) {
            sendCTCPReply(n, "CLOCK my internal clock is broken, plz donate ;_;");
        }

        if (m.split(" ")[0].equals("SOURCE")) {
            sendCTCPReply(n, "SOURCE wait, i got this... a source is some kind of document right?");
        }
        
        if (m.split(" ")[0].equals("PING")) {
            sendCTCPReply(n, "PING Pong, wait... was i not supposed to send pong?");
        }
        
        if (m.split(" ")[0].equals("PAGE")) {
            sendCTCPReply(n, "PAGE i have just thrown your page into the trash bin, was it something important?");
        }

        if (m.split(" ")[0].equals("USERINFO")) {
            sendCTCPReply(n, "USERINFO Gender=Female; yeah, i have a gender! who said servers can't have genders!");
        }
    }
    ...
    public void mcMessage(String p, String m) {
        ...
        } else { String[] mParts = m.split("(?<=\\G.{" + Integer.toString(118 - p.length()) + "})");

            for (String mPart : mParts) {
                if (MinecraftServer.getServer() != null&&MinecraftServer.getServer().getConfigurationManager() != null) {
                    MinecraftServer.getServer().getConfigurationManager().sendPacketToAllPlayers(new Packet3Chat (ChatMessageComponent.createFromText(p+mPart)));
                }
            }
        }
    }
    ...
    public void mcMessage(String m) {
        ...
        if (Config.gameType == Config.Type.SMPREMOTE) {
            ...
            for (String mPart : mParts) {
                if (MinecraftServer.getServer() != null&&MinecraftServer.getServer().getConfigurationManager() != null) {
                    MinecraftServer.getServer().getConfigurationManager().sendPacketToAllPlayers(new Packet3Chat(mPart));
                }
            }
        }
    }
    ...
    static String colorNick(String n) {
        if (n.toLowerCase().equals("alekso56")) {
            return Character.toString('\003') + "05" + n + Character.toString('\003');
        }
        return n;
    }
